"""
test_phase2_5c_benchmark.py
Phase 2.5c Performance Benchmarking Harness for JARVIS M7.
Measures wall-clock latency and per-agent-role latency across 3 scenarios (N=15 runs each).
Does NOT perform any optimizations or alter execution paths.
"""
...
def run_benchmarks(num_runs: int = 15):
    ...
    for i in range(num_runs):
        ...
        # Direct deterministic path evaluation
    ...
    for i in range(num_runs):
        req_id = f"req-bench-{i+1:03d}"
        dec = brain_router.route(cmd_multi)
        s3_routes.append(dec.route)

        start_t = time.time()
        # Measure per-role step timing via pipeline_timer event hooks
